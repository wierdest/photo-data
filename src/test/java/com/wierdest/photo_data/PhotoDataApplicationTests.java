...
class PhotoDataApplicationTests {
	...
	void helloParamShouldReturnDefaultMessage() {
		assertThat(
				this.testRestTemplate.getForObject("http://localhost:" + port + "/hello/param?message=", String.class))
	...
	void helloPathShouldReturnDefaultMessage() {
		String message = "Path";
		assertThat(this.testRestTemplate.getForObject("http://localhost:" + port + "/hello/path/{message}",
